...
public Map<String, String> editProfile(@RequestBody Member memberChg,HttpSession session) {
	...
	if(memberInfo!=null) {
		if(memberChg!=null) {
			...
			if(memberChg.getLastName()==null || memberChg.getLastName().trim().length()==0) {
				map.put("er013", "LastName not null");
			}
			if(memberChg.getBirthYear()==null)
			{	System.out.println(memberChg.getBirthYear());
				map.put("er014","birthYear not null");}
			if(memberChg.getPhone()==null || memberChg.getPhone().trim().length()==0) {
				map.put("er015", "phone not null");
			}if(memberChg.getCountry()==null || memberChg.getCountry().trim().length()==0) {
				map.put("er016", "country not null");
			}if(memberChg.getGender()==null ) {
...
@ResponseBody
@RequestMapping(path={"/showMemberProfile.controller"}, method={RequestMethod.GET},produces = { "application/json" })
public Member showMemberProfile(Integer mid,HttpSession session) {
	Member member = ms.getMemberInfo(mid);
...
public Map<String, Object> editPref(Member member,HttpSession session) {
	...
	if(memberInfo!=null) {
	if(member !=null) {		
		Integer currentLv = memberInfo.getCurrentLv();
		switch (currentLv)
		{
		case 1:
